    ...
    group('AC: 10-tap no consecutive repeats', () {
      test(
        ...
        for (int i = 0; i < 10; i++) {
          ...
          if (currentId != null) {
            expect(
              quote.id,
              isNot(equals(currentId)),
              reason:
                  'Tap $i: getRandomQuote must not return same quote as currentId',
            );
          }
          currentId = quote.id;
        }
      ...
      test('getRandomQuote distributes across catalog (not stuck on one quote)',
        ...
        expect(
          ...
          reason:
              'Random selection should distribute across catalog, not stay on same quote',
        );
      });
    ...
    group('Edge case: Single-quote catalog', () {
      test('getRandomQuote returns the only quote when catalog has 1 item', () {
        ...
        expect(
          ...
          greaterThan(1),
          reason:
              'Current catalog has multiple quotes, single-quote edge does not apply',
        );
      });
    });

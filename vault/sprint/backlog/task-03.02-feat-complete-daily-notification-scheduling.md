...
- **Async quote lookup after the database pivot:** Once task `04.02` lands, quote retrieval may no longer be immediate in-memory access. If schedule, reschedule, or verification paths start failing after that pivot, verify the notification flow awaits the repository-backed quote lookup before composing notification content.
